"""
Gemini Streaming CRM & Financial Bot
Main Application Entrypoint
Refactored into clean modular architecture:
- core/: settings, security, utilities, template engine
- db/: schema, connection pool, domain repositories
- services/: finance, templates, CSV backup/import/export
- mcp_server/: FastMCP tools organized by business domain
- routers/: FastAPI HTTP routes and OAuth 2.0 endpoints
- templates/: Clean HTML/CSS/JS files
"""
...
import mcp_server.tools  # Registers all 50 FastMCP tools with Gemini Spark
...
# Inicializar logger del sistema unificado
system_logger.setup_system_logging()
logger = logging.getLogger("main")

# Servidor FastMCP HTTP para Gemini Spark
mcp_app = mcp.http_app(path="/")


@asynccontextmanager
async def lifespan(app: FastAPI):
    """Ciclo de vida de la aplicación: inicialización de BD, admin, tareas y MCP."""
    # 1. Inicializar esquema de base de datos y migraciones
    database.init_db()
...
async def health():
    return {
        "status": "ok",
        "service": "streaming-crm-interactive-bot",
        "version": "3.1.0",
    }
